...
def analyze_trader_behavior(user_id: str, trades: list) -> dict:
    ...
    for sess_id, st in sessions.items():
        ...
        fomo_trades = [t for t in st if t.get("emotionalState") in ["greedy", "anxious"]]
        if fomo_trades:
        ...
        revenge_trades = [t for t in st if t.get("revengeFlag") is True or t.get("revengeFlag") == "true"]
        if revenge_trades:
