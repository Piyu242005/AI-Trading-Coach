...
if st.session_state.token:
    ...
    with tab1:
        ...
        if trades:
            ...
            fig = px.line(df, x='entryAt', y='cumulative_pnl', 
            ...
            fig2 = px.histogram(df, x="assetClass", color="outcome", barmode="group",
                                title="Trade Outcomes by Asset Class",
                                template="plotly_dark")
            st.plotly_chart(fig2, use_container_width=True)
            
            with st.expander("View Raw Trade Data"):
                st.dataframe(df[['tradeId', 'asset', 'direction', 'entryPrice', 'exitPrice', 'pnl', 'outcome']])
        else:
            st.info("No trades found for this user. Make sure the User ID is correct.")
            
    with tab2:
        st.header("Behavioral AI Profiling")
        st.markdown("We use advanced AI to detect hidden psychological patterns in your trading behavior.")
        
        if st.button("Generate Psychological Profile"):
            with st.spinner("Analyzing your trading history..."):
                headers = {"Authorization": f"Bearer {st.session_state.token}"}
                resp = requests.get(f"{API_URL}/api/profiling/{st.session_state.user_id}", headers=headers)
                
                if resp.status_code == 200:
                    profile = resp.json()
                    st.success("Profiling complete!")
                    
                    st.subheader(f"Detected Behavior: {profile.get('behavior', 'Unknown').replace('_', ' ').title()}")
                    st.write("**Summary:**", profile.get('summary', 'No summary provided.'))
    ...
    with tab3:
        ...
        st.markdown("Get actionable, real-time feedback based on your recent trading activity.")
        ...
        if len(st.session_state.trades_data) > 0:
            ...
            if st.button("Request Coaching Feedback"):
                with st.spinner("AI Coach is reviewing your trades..."):
                    ...
                    payload = {"trades": st.session_state.trades_data[-20:]} # send last 20 for payload limits
                    
                    resp = requests.post(f"{API_URL}/api/coaching/{st.session_state.user_id}", json=payload, headers=headers)
                    ...
                    if resp.status_code == 200:
                        ...
                        if signals:
                            ...
                            for s in signals:
                                st.warning(f"**{s['signal'].replace('_', ' ').title()}**: {s['reason']}")
                    else:
